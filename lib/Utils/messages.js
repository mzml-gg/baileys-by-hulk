...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('externalAdReply' in message && !!message.externalAdReply) {
        const externalAdReply = { ...message.externalAdReply };
        if (externalAdReply.thumbnail && typeof externalAdReply.thumbnail === 'string') {
            externalAdReply.thumbnail = Buffer.from(externalAdReply.thumbnail, 'base64');
        }
        if (externalAdReply.mediaType == null) {
            externalAdReply.mediaType = externalAdReply.thumbnail ? 1 : 0;
        }
        if (externalAdReply.showAdAttribution == null) {
            externalAdReply.showAdAttribution = true;
        }
        if (externalAdReply.renderLargerThumbnail == null) {
            externalAdReply.renderLargerThumbnail = false;
        }
        if (!message.contextInfo) {
            message = { ...message, contextInfo: {} };
        }
        message.contextInfo = { ...message.contextInfo, externalAdReply };
    }
    if ('contextInfo' in message && !!message.contextInfo) {
        const contextInfo = { ...message.contextInfo };
        if (contextInfo.externalAdReply) {
            const ext = contextInfo.externalAdReply;
            if (ext.thumbnail && typeof ext.thumbnail === 'string') {
                ext.thumbnail = Buffer.from(ext.thumbnail, 'base64');
            }
            if (ext.mediaType == null) {
                ext.mediaType = ext.thumbnail ? 1 : 0;
            }
            if (ext.showAdAttribution == null) {
                ext.showAdAttribution = true;
            }
            if (ext.renderLargerThumbnail == null) {
                ext.renderLargerThumbnail = false;
            }
        }
        const [messageType] = Object.keys(m);
